...
| `.github/workflows/docs-site.yml` | Builds the MkDocs Material site from `docs/public/` (config: `docs/mkdocs.yml`) and deploys it to GitHub Pages at docs.rulebeat.com. "Docs build" runs `mkdocs build --strict` on every PR (no `paths:` filter, same reasoning as `pr-checks.yml`), so a broken doc link or anchor fails the PR; the deploy job runs on docs pushes to `main` |
...
| `scripts/release-dependency-notes.mjs` | Derives a release's `### Dependencies` block by diffing the two published packages' runtime `dependencies` between the previous tag and the commit being released. Skips anything a human already named in `[Unreleased]`, and is idempotent |
| `scripts/recommend-bump.mjs` | Reads patch/minor/major off `[Unreleased]`'s content, and refuses an explicit override that is lower than the changelog requires |
| `scripts/release-dependency-notes.test.mjs` | `node:test` cases for both |
| `scripts/check-changelog-structure.mjs` | CHANGELOG.md's internal consistency: header/footer-link agreement, `[Unreleased]` pointing at the newest release, descending versions, real non-increasing dates, no duplicates or orphans. Called from `verify-release-version.mjs`'s `main()`, so it gates both tagging and promotion without touching either workflow |
| `scripts/check-changelog-structure.test.mjs` | `node:test` cases, including a fixture reproducing the real footer drift (`[Unreleased]` still comparing from v0.1.0 after 0.2.0 shipped) |
...
| `scripts/resolve-pr-context.sh` | Resolves either trigger shape `pr-checks.yml` supports (a `pull_request` event, or the `workflow_dispatch` `prepare-release.yml` fires) into the flat values the gate reads. Reads the payload with `jq` rather than `${{ }}` interpolation: `toJSON` pretty-prints, and a multi-line value makes `$GITHUB_OUTPUT` reject the whole step |
| `scripts/resolve-pr-context.test.mjs` | `node:test` cases running the real script against fixture payloads. Asserts every value is single-line whatever the labels contain, and that a branch name full of shell metacharacters is data |
| `scripts/verify-builder-deps.mjs` | Asserts every declared runtime dependency of `packages/core` and `packages/web` resolves from that workspace at the major its manifest declares. Run inside the Docker builder stage, where resolving to the wrong copy used to be silent |
| `scripts/verify-builder-deps.test.mjs` | `node:test` cases for it, including that a workspace-nested copy wins over a hoisted one, and that a CSS-only package with no JavaScript entry still resolves |
| `scripts/fetch-main-ref.sh` | Makes `origin/main` usable for `git merge-base` inside a CI job. `actions/checkout` fetches one ref at depth 1, so main is neither a ref nor a shared ancestor; this fetches it through an explicit refspec and unshallows only when the clone actually is shallow |
| `scripts/fetch-main-ref.test.mjs` | `node:test` cases against local git fixtures, including a guard asserting the fixture clone really is shallow, since a `--depth 1` clone from a plain path silently ignores the depth |
