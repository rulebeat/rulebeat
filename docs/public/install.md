...
docker run -d --name rulebeat --restart unless-stopped -p 127.0.0.1:3000:3000 \
  ...
  ghcr.io/rulebeat/rulebeat:0.5.0
```
...
services:
  rulebeat:
    image: ghcr.io/rulebeat/rulebeat:0.5.0
...
3. **Create the Container App.** Container Apps, Create; a new environment is fine. On the Container
   ...
   `ghcr.io/rulebeat/rulebeat:0.5.0`, which the portal splits into a registry login server of
...
1. `docker pull ghcr.io/rulebeat/rulebeat:0.5.0`
