...
describe('syncScanFindings — created / repeat / resolved / reactivated', () => {
  it('creates a new finding as active, firstSeenAt = lastSeenAt = the scan\'s finishedAt, timesSeen 1, one created event', () => {
  ...
  it('a finding seen again advances lastSeenAt and timesSeen, keeps firstSeenAt, and records no new event', () => {
    const f = finding('vm-1');
    const d1 = daysAgo(3);
    const d2 = daysAgo(2);
  ...
  it('a finding that stops appearing resolves: status fixed, resolvedAt set, one resolved event', () => {
    const f = finding('vm-1');
    const d1 = daysAgo(3);
    const d2 = daysAgo(2);
    syncScanFindings({ scanId: 's1', category: CATEGORY, ranRuleIds: [RULE_A], findings: [f], finishedAt: d1 });
  ...
  it('a fixed finding reappearing reactivates: status active, resolvedAt cleared, firstSeenAt preserved, one reactivated event', () => {
    const f = finding('vm-1');
    const d1 = daysAgo(3);
    const d2 = daysAgo(2);
    const d3 = daysAgo(1);
    syncScanFindings({ scanId: 's1', category: CATEGORY, ranRuleIds: [RULE_A], findings: [f], finishedAt: d1 });
    syncScanFindings({ scanId: 's2', category: CATEGORY, ranRuleIds: [RULE_A], findings: [], finishedAt: d2 });
    const result = syncScanFindings({ scanId: 's3', category: CATEGORY, ranRuleIds: [RULE_A], findings: [f], finishedAt: d3 });
...
describe('syncScanFindings — 500-row CHUNK_SIZE boundary', () => {
  it('classifies and resolves correctly across a findings list spanning two chunks (>500)', () => {
    const many = Array.from({ length: 501 }, (_, i) => finding(`vm-${i}`));
    const result = syncScanFindings({ scanId: 's1', category: CATEGORY, ranRuleIds: [RULE_A], findings: many, finishedAt: daysAgo(2) });
    expect(result.created).toHaveLength(501);
    expect(listFindings({ status: 'active' })).toHaveLength(501);
    ...
    const result2 = syncScanFindings({ scanId: 's2', category: CATEGORY, ranRuleIds: [RULE_A], findings: [many[0]!], finishedAt: daysAgo(1) });
    expect(result2.resolved).toHaveLength(500);
    expect(listFindings({ status: 'active' })).toHaveLength(1);
    expect(listFindings({ status: 'fixed' })).toHaveLength(500);
  });

  it('resolves stale findings correctly when ranRuleIds itself spans more than 500 ids', () => {
    ...
    syncScanFindings({ scanId: 's0', category: CATEGORY, ranRuleIds, findings: manyFindings, finishedAt: daysAgo(2) });
    expect(listFindings({ status: 'active' })).toHaveLength(501);

    const result = syncScanFindings({ scanId: 's1', category: CATEGORY, ranRuleIds, findings: [], finishedAt: daysAgo(1) });
    expect(result.resolved).toHaveLength(501);
    expect(listFindings({ status: 'active' })).toHaveLength(0);
  });
});

describe('deleteFindingsForRule', () => {
  it('removes both the findings row and its finding_events rows for that rule', () => {
    const f = finding('vm-1');
    ...
    expect(listFindings()).toHaveLength(0);
    expect(eventsFor(f.fingerprint)).toHaveLength(0);
  });
});

describe('event-retention pruning', () => {
  it('prunes a finding_events row older than 180 days on the next non-silent sync', () => {
    const f = finding('vm-old');
    syncScanFindings({ scanId: 's0', category: CATEGORY, ranRuleIds: [RULE_A], findings: [f], finishedAt: daysAgo(1) });
    expect(eventsFor(f.fingerprint)).toHaveLength(1);
    ...
    syncScanFindings({ scanId: 's1', category: CATEGORY, ranRuleIds: [RULE_A], findings: [], finishedAt: new Date().toISOString() });
...
describe('duplicate ARG rows sharing one fingerprint (RB-QA-019, fixed)', () => {
  it('a single scan returning the same resource twice counts as one sighting, not two', () => {
    const f = finding('vm-dup');
    const result = syncScanFindings({ scanId: 's1', category: CATEGORY, ranRuleIds: [RULE_A], findings: [f, f], finishedAt: daysAgo(1) });
  ...
  it('a duplicate row for a currently-fixed finding reactivates it exactly once', () => {
    ...
    syncScanFindings({ scanId: 's1', category: CATEGORY, ranRuleIds: [RULE_A], findings: [f], finishedAt: d1 });
    syncScanFindings({ scanId: 's2', category: CATEGORY, ranRuleIds: [RULE_A], findings: [], finishedAt: d2 });
    const result = syncScanFindings({ scanId: 's3', category: CATEGORY, ranRuleIds: [RULE_A], findings: [f, f], finishedAt: d3 });
  ...
  it('when duplicates differ in fields, the last occurrence in the array wins', () => {
    const f1 = finding('vm-dup-fields', { description: 'first observation' });
    const f2 = { ...f1, description: 'second observation' };
    syncScanFindings({ scanId: 's1', category: CATEGORY, ranRuleIds: [RULE_A], findings: [f1, f2], finishedAt: daysAgo(1) });

    const row = listFindings().find(r => r.fingerprint === f1.fingerprint)!;
    expect(row.description).toBe('second observation');
  });

  it('a duplicate for one fingerprint does not affect an unrelated fingerprint\'s resolved/reactivated classification', () => {
    const dup = finding('vm-dup-isolated');
    const other = finding('vm-other-isolated');
    const d1 = daysAgo(2);
    const d2 = daysAgo(1);
    syncScanFindings({ scanId: 's1', category: CATEGORY, ranRuleIds: [RULE_A], findings: [dup, other], finishedAt: d1 });
    const result = syncScanFindings({ scanId: 's2', category: CATEGORY, ranRuleIds: [RULE_A], findings: [dup, dup], finishedAt: d2 });
...
describe('active vs. expired suppressions (queryActiveFindings)', () => {
  it('a permanent suppression (no expiresAt) excludes the finding from active results', () => {
    const f = finding('vm-1');
    syncScanFindings({ scanId: 's1', category: CATEGORY, ranRuleIds: [RULE_A], findings: [f], finishedAt: '2026-01-01T00:00:00.000Z' });
  ...
  it('a suppression with a future expiresAt still excludes the finding', () => {
    const f = finding('vm-1');
    syncScanFindings({ scanId: 's1', category: CATEGORY, ranRuleIds: [RULE_A], findings: [f], finishedAt: '2026-01-01T00:00:00.000Z' });
  ...
  it('an expired suppression no longer excludes the finding', () => {
    const f = finding('vm-1');
    syncScanFindings({ scanId: 's1', category: CATEGORY, ranRuleIds: [RULE_A], findings: [f], finishedAt: '2026-01-01T00:00:00.000Z' });
    ...
    expect(queryActiveFindings(FILTERS).map(r => r.fingerprint)).toContain(f.fingerprint);
  });

  it('filters.includeSuppressed bypasses even a permanent suppression', () => {
    const f = finding('vm-1');
    syncScanFindings({ scanId: 's1', category: CATEGORY, ranRuleIds: [RULE_A], findings: [f], finishedAt: '2026-01-01T00:00:00.000Z' });
    saveSuppressions([suppression({ fingerprint: f.fingerprint, resourceId: f.resourceId })]);
