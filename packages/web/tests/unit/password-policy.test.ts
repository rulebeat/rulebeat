...
describe('validatePasswordStrength', () => {
  ...
  it('accepts a password exactly at the minimum length that is not blocklisted', async () => {
    const result = validatePasswordStrength('correct-horse-b'); // 16 chars, not in the list
    expect(result.ok).toBe(true);
  });

  it('rejects a blocklisted password even though it clears the length floor', async () => {
    const result = validatePasswordStrength('password12345678');
    expect(result.ok).toBe(false);
  });

  it('blocklist matching is case-insensitive', async () => {
    const result = validatePasswordStrength('PASSWORD12345678');
    expect(result.ok).toBe(false);
  });

  it('every blocklist entry is itself long enough to prove the length floor', async () => {
  ...
  it('accepts a genuinely strong, non-blocklisted password', async () => {
