...
describe('notification-deliveries repository', () => {
  ...
  async function makeChannel(name = 'Test channel'): Promise<string> {
    return (await createChannel({ name, type: 'webhook', url: 'https://example.test/hook' })).id;
  }

  afterEach(async () => {
    if (channelId) await deleteChannel(channelId); // idempotent — a test may have already deleted it
  });

  it('records a delivery and lists history newest-first', async () => {
    channelId = await makeChannel();
    await recordDelivery({ channelId, scheduleId: 's1', runId: 'r1', ok: true, attempts: 1, httpStatus: 200, findingsCount: 3 });
    await recordDelivery({ channelId, scheduleId: 's1', runId: 'r2', ok: false, attempts: 3, httpStatus: 500, error: 'HTTP 500', findingsCount: 1 });
  ...
  it('lists newest-first even when two deliveries land the same millisecond occurredAt', async () => {
  ...
  it('caps history at 50 rows per channel', async () => {
    channelId = await makeChannel();
    for (let i = 0; i < 55; i++) {
      await recordDelivery({ channelId, scheduleId: 's1', runId: `r${i}`, ok: true, attempts: 1, findingsCount: 0 });
    }
    ...
    const history = await listDeliveriesForChannel(channelId, 100);
    expect(history).toHaveLength(50);
  });

  it('deleteDeliveriesForChannel removes history for that channel only', async () => {
    channelId = await makeChannel();
    const otherId = await makeChannel('Other channel');
    try {
      await recordDelivery({ channelId, scheduleId: 's1', runId: 'r1', ok: true, attempts: 1, findingsCount: 0 });
      await recordDelivery({ channelId: otherId, scheduleId: 's1', runId: 'r2', ok: true, attempts: 1, findingsCount: 0 });

      await deleteDeliveriesForChannel(channelId);

      expect(await listDeliveriesForChannel(channelId)).toHaveLength(0);
      expect(await listDeliveriesForChannel(otherId)).toHaveLength(1);
    } finally {
      await deleteChannel(otherId);
    }
  });

  it('deleting a channel cascades its delivery history', async () => {
    channelId = await makeChannel();
    await recordDelivery({ channelId, scheduleId: 's1', runId: 'r1', ok: true, attempts: 1, findingsCount: 0 });
