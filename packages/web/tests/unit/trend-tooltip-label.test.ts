...
describe('formatTrendTooltipLabel (spec 033)', () => {
  it('reads passing/total off the prefixed per-category fields when present', () => {
  ...
  it('falls back to the unprefixed fields for the single-category chart shape', () => {
    const payload = { date: '2026-08-01', pct: 25, passing: 1, total: 4 };
    expect(formatTrendTooltipLabel('pct', 25, payload)).toBe('1/4 passing (25%)');
  });

  it('falls back to a bare percentage when no passing/total is on the row at all', () => {
