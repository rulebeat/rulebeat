...
export function resolveRulesForSchedule(schedule: Pick<Schedule, 'targetType' | 'targetValues'>): Rule[] {
  const enabledRules = loadRules().filter(r => r.enabled) as unknown as Rule[];
...
export function resolveCategoriesForSchedule(schedule: Pick<Schedule, 'targetType' | 'targetValues'>): string[] {
  if (schedule.targetType === 'all') return listCategories().map(c => c.id);
  ...
  return [...new Set(resolveRulesForSchedule(schedule).map(r => r.category))];
}

export function getNextRunForCategory(slug: string): string | null {
  const candidates = listSchedules()
    .filter(s => s.enabled && s.nextRunAt !== null && resolveCategoriesForSchedule(s).includes(slug))
    .map(s => s.nextRunAt!);
